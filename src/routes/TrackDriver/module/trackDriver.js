...
export function getDriverInfo() {
  return (dispatch, store) => {
    ...
    request
      .get('http://192.168.0.41:3000/api/driver/' + id)
      .finish((erroe, res) => {
        dispatch({
...
export function getDriverLocation() {
  return (dispatch, store) => {
    let id = store().home.booking.driverId;
    request
      .get('http://192.168.0.41:3000/api/driverLocation/' + id)
      .finish((erroe, res) => {
        dispatch({
...
function handleGetCurrentLocation(state, action) {
  // console.log('trackDriver: handleGetCurrentLocation: lat=',action.payload.coords.latitude,',long=',action.payload.coords.longitude);
  return update(state, {
    region: {
      latitude: {
